// Refactored from LiveScript - removed IIFE, var→const/let (conservative refactor, kept curry$ and slice$/toString$ for now)
...
const { lines, unlines } = require('./utils');
...
let getQueryEngine;  // Forward declaration - used in run.search/run.replace before being defined
const slice$ = [].slice;
...
run = function(arg$){
    ...
    if (that = options.replaceFunc) {
      ...
      try {
        replacement = fs.readFileSync(that, 'utf8').replace(/([\s\S]*?)\r?\n$/, '$1');
      } catch (e$) {
        e = e$;
    ...
    search = function(name, input){
      ...
      count = (that = options.maxCount) != null ? Math.min(that, resultsLen) : resultsLen;
      sortedResults = [...results].sort(resultsSortFunc);
        ...
        try {
          if (target === '-') {
            ...
            if (stat.isDirectory() && options.recursive) {
              ...
              displayPath = path.relative(basePath, targetPath).replace(/\\/g, '/');
