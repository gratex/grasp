...
function generateSyntaxHelp() {
  ...
  const syntaxInfoStrings = syntaxInfo.map(nodesInfo => {
...
function generateSyntaxHelpForNode(nodeName) {
  ...
  let syntaxStr = '';
  if (syntax) {
    syntaxStr = "\nsyntax:\n" + unlines(lines(syntax).map(line => "  " + line));
  }
  ...
  return nameStr + "\n" + "=".repeat(nameStr.length) + strs.join('') + syntaxStr + exampleStr + noteStr;
}
...
function generateCategoryHelp() {
  ...
  for (const [alias, category] of Object.entries(matchesAliasMap)) {
    ...
    const names = fullNodeNames.map(node => syntaxFlat[node].alias);
...
function generateHelpForCategory(name) {
  ...
  const names = fullNodeNames.map(node => syntaxFlat[node].alias + " (" + node + ")");
